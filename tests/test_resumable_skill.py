...
class QualityProfileTests(unittest.TestCase):
    ...
    def test_balanced_batch_records_but_does_not_block_minor_hard_boundary(self):
        report = {
            "chunks": [
                {
                    "chunk_id": "chunk_001",
        ...
        gated = audiobook_workflow.performance_gate(report, "balanced", phase="batch")
        self.assertEqual(gated["gate_failed_chunk_ids"], [])
        self.assertEqual(gated["gate_status"], "pass")

    def test_balanced_pilot_blocks_minor_hard_boundary(self):
        report = {
            "chunks": [{
                "chunk_id": "chunk_001",
                "status": "fail",
        ...
        gated = audiobook_workflow.performance_gate(report, "balanced", phase="pilot")
        self.assertEqual(gated["gate_failed_chunk_ids"], ["chunk_001"])
        self.assertEqual(gated["gate_status"], "fail")

    def test_balanced_pilot_blocks_minor_missing_required_sound_layer(self):
        report = {
            "chunks": [{
                "chunk_id": "chunk_001",
                "status": "fail",
                "issues": [{"type": "missing_required_ambience", "severity": "minor"}],
            }],
            "unavailable_chunk_ids": [],
        }
        gated = audiobook_workflow.performance_gate(report, "balanced", phase="pilot")
