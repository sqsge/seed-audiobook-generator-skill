...
VOICE_CANDIDATES = [
    ...
    "en_male_excited-male-voice_uranus_bigtts",
    "en_male_adam-imitation_uranus_bigtts",
    "en_female_sharron_uranus_bigtts",
    "en_male_deep-voice_uranus_bigtts",
    "en_male_father-christmas_uranus_bigtts",
    "en_male_david_uranus_bigtts",
    "en_female_jane_uranus_bigtts",
    "en_female_nadia_uranus_bigtts",
]
...
"""
    ...
    response = llm_chat.chat_text(
        ...
        model=os.getenv("SEED_AUDIO_REWRITE_MODEL", os.getenv("LLM_MODEL", "dola-seed-2-1-turbo-260628")),
        temperature=0.1,
        max_tokens=6000,
    )
    if planning_dir:
...
def write_runner_log_attempt(log_dir: Path, stem: str, stdout: str, stderr: str) -> dict[str, str | int]:
    """Store every runner invocation under a new immutable attempt number."""
    log_dir.mkdir(parents=True, exist_ok=True)
    pattern = re.compile(rf"{re.escape(stem)}\.attempt_(\d+)\.(?:stdout|stderr)\.txt")
    existing = [
        int(match.group(1))
        for path in log_dir.glob(f"{stem}.attempt_*.txt")
        if (match := pattern.fullmatch(path.name))
    ]
    if (log_dir / f"{stem}.stdout.txt").exists() or (log_dir / f"{stem}.stderr.txt").exists():
        existing.append(1)
    attempt = max(existing, default=0) + 1
    stdout_path = log_dir / f"{stem}.attempt_{attempt:03d}.stdout.txt"
    stderr_path = log_dir / f"{stem}.attempt_{attempt:03d}.stderr.txt"
    write_text_once(stdout_path, stdout)
    write_text_once(stderr_path, stderr)
    # Preserve the original compatibility names as immutable first-attempt evidence.
    write_text_once(log_dir / f"{stem}.stdout.txt", stdout)
    write_text_once(log_dir / f"{stem}.stderr.txt", stderr)
    return {
        "attempt": attempt,
    ...
    for section in state["sections"]:
        ...
        if section.get("status") == "needs_review":
            ...
            if rebuild_director_prompts:
                ...
                write_runner_log_attempt(logs, f"{section_id}.rebuild", rebuild.stdout, rebuild.stderr)
        ...
        try:
            ...
            raise
        logs = run_dir / "runner_logs"
        write_runner_log_attempt(logs, section_id, result.stdout, result.stderr)
